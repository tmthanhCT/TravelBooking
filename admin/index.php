...
// Doanh thu booking theo tháng (12 tháng)
$monthlyRevenue = array_fill(1, 12, 0);
$stmt = $pdo->query("SELECT MONTH(booking_at) as m, SUM(subtotal) as total FROM bookings WHERE YEAR(booking_at) = YEAR(CURDATE()) GROUP BY m");
foreach ($stmt as $row) {
    $monthlyRevenue[(int)$row['m']] = (float)$row['total'];
}
// Thống kê trạng thái booking
$statusCount = ['pending'=>0, 'confirmed'=>0, 'cancelled'=>0];
$stmt2 = $pdo->query("SELECT STATUS, COUNT(*) as cnt FROM bookings GROUP BY STATUS");
foreach ($stmt2 as $row) {
    $status = strtolower($row['STATUS']);
    if (isset($statusCount[$status])) $statusCount[$status] = (int)$row['cnt'];
}
?>
        ...
        <div class="col-xl-8 col-lg-7">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <script>
                    // Dữ liệu doanh thu booking theo tháng
                    var bookingRevenue = <?= json_encode(array_values($monthlyRevenue)) ?>;
                    </script>
                </div>
            </div>
        </div>
        ...
        <div class="col-xl-4 col-lg-5">
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <script>
                    // Dữ liệu trạng thái booking
                    var bookingStatus = <?= json_encode(array_values($statusCount)) ?>;
                    </script>
...
<script>
// --- Area Chart: Doanh thu booking ---
if (typeof bookingRevenue !== 'undefined') {
            ...
            tooltips: {
                ...
                callbacks: {
                    label: function(tooltipItem, chart) {
                        return 'Doanh thu: $' + tooltipItem.yLabel.toLocaleString();
                    }
                }
            }
        }
    ...
    new Chart(ctxPie, {
        ...
        data: {
            ...
            datasets: [{
                ...
                hoverBackgroundColor: ['#f4b619', '#17a673', '#c82333'],
